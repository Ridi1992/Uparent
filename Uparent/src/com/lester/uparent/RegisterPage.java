...
//				btnNext.setBackgroundResource(resId);
		...
		if (data != null) {
			...
			if (page == 1) {
				...
				if (res != null && phoneMap != null) {
					...
					intent.setClass(mycContext, GetPassWord.class);
	...
	private void getValidate() {
		...
		if(Net.isNetworkAvailable(getContext())){
			gethandler.sendMessage(gethandler.obtainMessage(404, "亲，你断网了~"));
			return;
		}
		new Thread(new Runnable() {
	        public void run() {
	            String service = "parentService";
	            String method = "validateGetPassword";
	            String token = MD5.MD5(service+method+Constants.Key);
	            String params = 
	            		"{loginId:'"+etPhoneNum.getText().toString()+"'}";
...
//	    				Object  object=JsonUtil.instance().fromJson(jsonData, new TypeToken<ArrayList<Sendlogdata.datas>>(){}.getType());
	        			gethandler.sendMessage(gethandler.obtainMessage(Constants.validateGetPassword, "验证通过"));
	    			}else{
	    				gethandler.sendMessage(gethandler.obtainMessage(404, jsonObj.getString("message")));
	    			}
	                // 显示响应
	            } catch (Exception e) {
	                e.printStackTrace();
		...
		Handler gethandler = new Handler(){
			public void handleMessage(android.os.Message msg) {
				try {
					switch (msg.what) {
					case Constants.validateGetPassword://验证通过
						System.out.println("验证通过");
						showDialog(myphone, mycode);
						break;
				
					case 404:
						Toast.makeText(getContext(), msg.obj.toString(), 0).show();
						break;
					}
				} catch (Exception e) {
