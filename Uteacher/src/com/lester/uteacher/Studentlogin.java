 ...
 *接送登记
 */
public final class Studentlogin extends Activity implements OnClickListener {
		...
		mAdapter=new BaseAdapter() {
			...
			public View getView(final int position, View convertView, ViewGroup parent) {
				...
				if(list.get(position).getPhoto()!=null &&
							 (!list.get(position).getPhoto().equals(""))){
						new Loadhead(Studentlogin.this,list.get(position).getPhoto(), headimg).execute();
					 }
			...
			view.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if(Cancheck){
						...
						Toast.makeText(Studentlogin.this, "带班老师只能查看学生状态，不能进行接送登记", 0).show();
					}
				}
			});
			break;
	...
	//刷新数据
	public void refresh(){
		list.get(logposition).setPosition_status(3);
...
//				Toast.makeText(MyStudent.this, "End of List!", Toast.LENGTH_SHORT).show();
	...
	Handler handler=new Handler(){
		public void handleMessage(android.os.Message msg) {
			...
			case Constants.GETSTUDENTDATA:
				list=(List<datas>) msg.obj;
				if(list==null||list.size()==0){
					Toast.makeText(getApplicationContext(), "没有记录", 0).show();
				}else{
					mAdapter.notifyDataSetChanged();
				}
				break;
			case Constants.GETCLASS:
				...
				if(classdatas!=null&&classdatas.size()!=0){
					if(classdatas.get(0).getSchool_id()!=null){
						...
						searchByClazzId();
						Cancheck=true;//带班老师只能查看学生状态，不能改变学生状态
	...
	private void getclass() {
		...
		lldialog=LodingDialog.DialogFactor(Studentlogin.this, "正在获取数据", true);
		new Thread(new Runnable() {
	        public void run() {
	            String service = "clazzService";
	            String method = "search";
	            String token = MD5.MD5(service+method+Constants.Key);
	            String params = "{loginId:'"+Userinfo.getmobile_phone(Studentlogin.this)+"'}";
