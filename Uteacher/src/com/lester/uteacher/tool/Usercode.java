...
public class Usercode {
	...
	public void saveQrCodePicture(Context context, Bitmap qrCodeBitmap) {
		File file1=new File("/sdcard/Uteacher/user/");
		if(!file1.exists()){
			file1.mkdirs();//不存在则新建路径
		}
		// 创建一个位于SD卡上的文件
		String imgname="code"+GetShare.user_id(null)+".jpg";
		File qrImage = new File("/sdcard/Uteacher/user/",imgname);
		if(qrImage.exists())
		{
			qrImage.delete();
		}
	...
	public String  getCodePath(){
		if(haveCode()){
			String str=Environment.getExternalStorageDirectory().getAbsolutePath()+"/Uteacher/user/"+"code"+GetShare.user_id(null)+".jpg";
			return str;
		}else{
			return "";
		}
